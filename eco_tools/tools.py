    ...
    def decorator(fun):
        ...
        def _wrapped_fun(self, request, *args, **kwargs):
            ...
            if datetime.strptime(payload.get('refresco'), "%m/%d/%Y %H:%M:%S") < datetime.now():
                return Response(data="{'mensaje': 'token expirado, loguéese de nuevo'}",
                         status=status.HTTP_401_UNAUTHORIZED)

            if datetime.strptime(payload.get('vencimiento'), "%m/%d/%Y %H:%M:%S") < datetime.now():
                return Response(data="{'mensaje': 'refresque el token e intente nuevamente'}",
            ...
            return Response(data="{'mensaje': 'no tiene el rol necesario para acceder a este recurso'}",
                            status=status.HTTP_401_UNAUTHORIZED)
        return _wrapped_fun
    return decorator
